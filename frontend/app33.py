...
def parse_sse_payload(raw_payload: str) -> dict[str, str]:
  ...
  if isinstance(payload, dict):
    ...
    # /chat/stream은 {"token": "..."} 형태로 오므로 UI 공통 포맷으로 맞춘다.
    if "delta" not in normalized and "token" in normalized:
      normalized["type"] = "token"
      normalized["delta"] = normalized["token"]
    return normalized
  return {"type": "token", "delta": str(payload)}

def stream_backend_response(endpoint: str, message: str, status_area, message_area) -> str:
  """선택된 endpoint에서 SSE를 받아 본문과 상태 표시를 갱신합니다."""
  collected = ""
...
mode = st.sidebar.radio("모드", ["일반 챗봇", "멀티에이전트"])
